...
def test_classrepr(Nums):
    assert re.match(r"<class bitsets\.meta\.bitset_list\('Nums', \(1, 2, 3, 4, 5, 6\), 0x[0-9a-fA-F]+, BitSet, List, Tuple\)>",
                    repr(Nums.List))
    assert re.match(r"<class bitsets\.meta\.bitset_tuple\('Nums', \(1, 2, 3, 4, 5, 6\), 0x[0-9a-fA-F]+, BitSet, List, Tuple\)>",
                    repr(Nums.Tuple))
